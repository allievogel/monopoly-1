...
Monopoly.initDice = function(){ // let's the dice roll
    $(".dice").click(function(){
...
Monopoly.updatePlayersMoney = function(player,amount){
    ...
    if (playersMoney < 0 ){
        ...
        var audio = new Audio('woopwoop.wav');
        audio.play();
    }
...
Monopoly.setNextPlayerTurn = function(){
    ...
    if (Monopoly.doubleCounter == 0); // this means that the two dice are not the same and we would want to continue with the next player
        var nextPlayerId = playerId + 1;
        if (nextPlayerId > $(".player").length){
        nextPlayerId = 1;
    }

    else nextPlayerId = playerId;
...
Monopoly.handleCommunityCard = function(player){
    var popup = Monopoly.getPopup("chance");
    popup.find(".popup-content").addClass("loading-state");
    $.get("https://itcmonopoly.appspot.com/get_random_comuunity_card", function(chanceJson){
        popup.find(".popup-content #text-placeholder").text(chanceJson["content"]);
        popup.find(".popup-title").text(chanceJson["title"]);
        popup.find(".popup-content").removeClass("loading-state");
        popup.find(".popup-content button").attr("data-action",chanceJson["action"]).attr("data-amount",chanceJson["amount"]);
